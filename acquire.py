...
from finance.prospects import ProspectCalculator, ProspectReader, ProspectDiscarding, ProspectAltering, ProspectWriter, ProspectTable, ProspectHeader, ProspectLayout, ProspectProtocols, ProspectProtocol
...
class AcquisitionTiming(object, fields=["date", "present", "tenure"], metaclass=NamedMeta): pass
...
class AcquisitionProtocols(ProspectProtocols, named={"trading": AcquisitionTrading, "timing": AcquisitionTiming}, metaclass=NamedMeta):
    ...
    def timeout(self, table): return (pd.to_datetime(self.timing.present) - table["current"]) >= self.timing.tenure
    ...
    def accept(self, table): return self.limited(Variables.Status.PENDING & self.liquid(table))
    @ProspectProtocol(Variables.Status.REJECTED, order=4)
    def reject(self, table): return self.limited(Variables.Status.PENDING & self.illiquid(table))
...
def main(*args, protocol={}, criterion={}, parameters={}, **kwargs):
    ...
    prospect_altering = ProspectAlteringRoutine(name="ProspectAltering", table=acquisition_table, protocol=[acquisition_protocols.pursue])
...
if __name__ == "__main__":
    ...
    sysPresent = Datetime(year=2024, month=11, day=6, hour=21, minute=0)
    sysDate = Date(year=2024, month=11, day=6)
    sysTenure = TimeDelta(days=1)
    sysTiming = dict(date=sysDate, present=sysPresent, tenure=sysTenure)
    sysTrading = dict(discount=2.00, liquidity=25, capacity=5)
