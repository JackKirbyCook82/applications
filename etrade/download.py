...
from finance.variables import DateRange, Symbol, Contract
...
class ContractQueue(Queues.FIFO, query=Contract): pass
...
def security(*args, source, destination, reader, parameters, **kwargs):
    security_schedule = Schedule(name="SecuritySchedule", source=source)
    security_downloader = ETradeMarketDownloader(name="SecurityDownloader", feed=reader)
    security_saver = Saver(name="SecuritySaver", destination=destination, query="contract")
...
def main(*args, apikey, apicode, tickers, **kwargs):
    ...
    with ETradeReader(name="SecurityReader", authorizer=security_authorizer) as security_reader:
        ...
        security_parameters = dict(source=contract_queue, destination={options_file: "w"}, reader=security_reader)
